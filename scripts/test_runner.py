...
def local():
    ...
    subprocess.run(["pytest", "tests/test_mcp_meta.py", "tests/test_trace_verification.py", "-k", "local", "-v"], check=True)
...
def remote():
    ...
    subprocess.run(["pytest", "tests/test_trace_verification.py", "-k", "remote", "-v", "-s"], check=True)
...
def verify_traces():
    ...
    print(f"Verifying traces in project '{project_id}' from the last {minutes} minutes...")
