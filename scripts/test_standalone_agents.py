...
def test_agent(name, dir_path):
    print(f"\n--- Testing {name} ---")
    import importlib.util
    
    # Load dynamically like deploy_agent_engine.py
    module_name = f"main_{name}"
    spec = importlib.util.spec_from_file_location(module_name, os.path.join(project_root, dir_path, "main.py"))
...
async def main():
    # Test RootRouter
    root_router = test_agent("RootRouter", "agents/RootRouter")
    
    # Test BookingOrchestrator
    booking_orchestrator = test_agent("BookingOrchestrator", "agents/BookingOrchestrator")
    ...
    if booking_orchestrator:
        ...
        for tool in booking_orchestrator.tools:
            ...
            if hasattr(tool, "__name__") and tool.__name__ == "calculate_trip_cost":
                ...
                break
            elif hasattr(tool, "func") and hasattr(tool.func, "__name__") and tool.func.__name__ == "calculate_trip_cost":
        ...
        if calculate_trip_cost:
            try:
                ...
                result = await calculate_trip_cost(flight_cost=100, hotel_cost=50, car_cost=30, days=2, loyalty_tier="Gold")
