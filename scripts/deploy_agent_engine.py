...
flags.DEFINE_string(
    "custom_domain", None, "Base custom domain for stable service URLs."
)
flags.DEFINE_string(
    "psc_network_attachment", None, "PSC Network Attachment for VPC egress."
)
...
def create_agent(
    ...
    # Dynamically export requirements from uv workspace to ensure consistency
    import re
    import subprocess

    requirements = []
    try:
        print("📦 Exporting requirements via uv...")
        result = subprocess.run(
            ["uv", "export", "--no-dev", "--frozen"],
            capture_output=True,
            text=True,
            check=True,
        )
        for line in result.stdout.splitlines():
            ...
            if not line or line.startswith("#"):
                continue
            # Match package==version (ignoring hashes and continuation characters)
            match = re.match(r"^([a-zA-Z0-9_-]+==[0-9a-zA-Z.-]+)", line)
            if match:
                requirements.append(match.group(1))
        print(f"📦 Successfully exported {len(requirements)} requirements.")
    except Exception as e:
        print(f"⚠️ Warning: Failed to export requirements via uv: {e}")
        print("⚠️ Falling back to minimal hardcoded list.")
        requirements = [
            "google-adk==2.0.0a2",
            "fastapi>=0.135.1",
            "mcp>=1.26.0",
    ...
    if custom_domain:
        ...
        urls = {
            ...
            "FLIGHT_SPECIALIST_AUDIENCE": service_urls.get(
                "flight_specialist_audience", ""
            ),
            "WEATHER_SPECIALIST_AUDIENCE": service_urls.get(
                "weather_specialist_audience", ""
            ),
            "PROFILE_MCP_AUDIENCE": service_urls.get("profile_mcp_audience", ""),
        }
    else:
        print(
            f"  Warning: No custom_domain or service_urls provided for {agent_obj.name}"
        )
    ...
    audience_map = {}
    if service_urls:
        if service_urls.get("flight_specialist_url") and service_urls.get(
            "flight_specialist_audience"
        ):
            audience_map[service_urls["flight_specialist_url"]] = service_urls[
                "flight_specialist_audience"
            ]
        if service_urls.get("weather_specialist_url") and service_urls.get(
            "weather_specialist_audience"
        ):
            audience_map[service_urls["weather_specialist_url"]] = service_urls[
                "weather_specialist_audience"
            ]
        if service_urls.get("profile_mcp_url") and service_urls.get(
            "profile_mcp_audience"
        ):
            audience_map[service_urls["profile_mcp_url"]] = service_urls[
                "profile_mcp_audience"
            ]
    ...
    if psc_network_attachment:
        if not vpc_project_id or not vpc_name:
            print(
                "  Warning: psc_network_attachment provided but vpc_project_id or vpc_name is missing. Skipping PSC config."
            )
        else:
            ...
            print(
                f"  Configuring Agent with PSC interface attached to {psc_network_attachment}"
            )
    ...
    existing_resource_name = (
        existing_agents_lookup.get(agent_obj.name) if existing_agents_lookup else None
    )
    try:
        if existing_resource_name:
            print(
                f"🤖 Updating existing agent {agent_obj.name} ({existing_resource_name})..."
            )
        ...
        try:
            ...
            for agent in remote_agents:
                if agent.display_name == agent_obj.name:
                    print(
                        f"  ✅ Recovered resource name for {agent_obj.name}: {agent.resource_name}"
                    )
...
def create(
    ...
    if os.path.exists(outputs_path):
        try:
            with open(outputs_path) as f:
                ...
                print(
                    f"Loaded {len(existing_agents_lookup)} existing agents from agent_engine_outputs.json (fast fallback)"
                )
        except Exception as e:
    ...
    if not existing_agents_lookup:
        try:
            remote_agents = agent_engines.AgentEngine.list()
            existing_agents_lookup = {
                agent.display_name: agent.resource_name
                for agent in remote_agents
                if getattr(agent, "display_name", "")
            }
...
def _main(argv: list[str] | None = None) -> None:
    ...
    location = (
        FLAGS.location
        if FLAGS.location
        else os.getenv("GOOGLE_CLOUD_LOCATION", os.getenv("REGION", "us-central1"))
    )
    bucket = FLAGS.bucket if FLAGS.bucket else os.getenv("GOOGLE_CLOUD_STORAGE_BUCKET")
    custom_domain = (
        FLAGS.custom_domain if FLAGS.custom_domain else os.getenv("CUSTOM_DOMAIN")
    )
    psc_network_attachment = (
    ...
    vpc_project_id = (
        FLAGS.vpc_project_id if FLAGS.vpc_project_id else os.getenv("VPC_PROJECT_ID")
    )
    vpc_name = FLAGS.vpc_name if FLAGS.vpc_name else os.getenv("VPC_NAME")
    ...
    if not custom_domain:
        print(
            "No custom domain set. Will read service URLs from terraform outputs or env vars."
        )
