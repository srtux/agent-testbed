...
async def get_hotel_inventory(destination: str, ctx: Context) -> dict:
    ...
    with tracer.start_as_current_span("mcp.tool_call.get_hotel_inventory", context=extract_trace_context(ctx)) as span:
...
async def get_weather(location: str, ctx: Context) -> dict:
    ...
    with tracer.start_as_current_span("mcp.tool_call.get_weather", context=extract_trace_context(ctx)) as span:
...
async def commit_booking(user_id: str, flight_id: str = "", hotel_id: str = "", car_id: str = "", ctx: Context = None) -> dict:
    ...
    with tracer.start_as_current_span("mcp.tool_call.commit_booking", context=extract_trace_context(ctx)) as span:
