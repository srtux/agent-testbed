locals {
  ...
  flight_specialist_url = local.use_custom_domain ? "https://flight-specialist.${var.custom_domain}" : "https://${google_cloud_run_v2_service.flight_specialist.name}-${data.google_project.current.number}.${var.region}.run.app"
  weather_specialist_url = local.use_custom_domain ? "https://weather-specialist.${var.custom_domain}" : "https://${google_cloud_run_v2_service.weather_specialist.name}-${data.google_project.current.number}.${var.region}.run.app"
  profile_mcp_url       = local.use_custom_domain ? "https://profile-mcp.${var.custom_domain}" : "https://${google_cloud_run_v2_service.profile_mcp.name}-${data.google_project.current.number}.${var.region}.run.app"
