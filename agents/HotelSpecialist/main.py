...
async def chat_endpoint(request: HotelRequest):
    logger.info(
        f"Hotel Specialist coordinating lodging at {request.destination} for user {request.user_id}"
    )
    ...
    logger.info(
        f"Hotel Specialist completed for user {request.user_id}. Response: {final_response}"
    )
